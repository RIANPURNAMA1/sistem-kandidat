...
function SuccessView({
  ...
  desc = 'Akun Anda telah berhasil dibuat. Silakan cek email untuk informasi lebih lanjut.',
  btnLabel = 'Masuk ke Akun',
  linkTo = '/login',
}: {
  title?: string
  desc?: string
  ...
  return (
    <div className="text-center animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <h1 className="text-[22px] font-bold mb-2 text-slate-800">{title}</h1>
...
function ModernLayout({
  ...
  const testimonial = {
    quote: 'Platform terbaik yang pernah saya gunakan. Proses cepat dan support sangat responsif.',
    name: 'Rina Kusuma',
  ...
  const perks = [
    { icon: TrendingUp, label: 'Terpercaya', value: 'Platform resmi & berpengalaman' },
    { icon: Shield, label: 'Aman & Nyaman', value: 'Data terenkripsi & aman' },
    { icon: Users, label: 'Komunitas Aktif', value: '2,500+ member aktif' },
  ]
  ...
  if (success) return wrapSuccess(
    <SuccessView title={successTitle} desc={successDesc} btnLabel={successBtn} linkTo={successLink} />
  )
  ...
  return (
    <div className="min-h-screen flex" style={{ background: '#F0F9FF' }}>
      ...
      >
        ...
        <div className="absolute bottom-16 -left-12 h-48 w-48 rounded-full opacity-10" style={{ background: '#fff' }} />
        ...
        <div className="relative z-10">
          ...
          <div className="mb-10">
            ...
            <p className="text-[13px] text-white/75 leading-relaxed max-w-[260px]">
              {sidebarDesc || 'Bergabunglah dan raih kesempatan terbaik Anda.'}
            </p>
          </div>
        ...
        <div className="relative z-10 bg-white/15 backdrop-blur-sm rounded-2xl p-5 border border-white/20">
          ...
          <p className="text-[12px] text-white/90 leading-relaxed mb-4 italic">&ldquo;{testimonial.quote}&rdquo;</p>
        ...
        <div className="w-full max-w-[480px]">
          <div className="rounded-sm shadow-sm border border-slate-100 p-8 lg:p-10">
            ...
            {refCode && (
              ...
              >
                Kode Referral aktif: <strong className="font-bold">{refCode}</strong>
              </div>
            )}
          ...
          <p className="text-[12px] text-slate-400 text-center mt-5">
            Sudah memiliki akun?{' '}
            <Link to="/login" className="font-semibold hover:underline transition-colors" style={{ color: '#009CE1' }}>
              Masuk di sini
            </Link>
          </p>
        </div>
  ...
  if (success) {
    return (
      <div className="min-h-screen flex items-center justify-center p-4 bg-slate-50">
        <div className="w-full max-w-[420px] rounded-2xl shadow-lg border border-slate-200 p-8 bg-white">
          <SuccessView title={successTitle} desc={successDesc} btnLabel={successBtn} linkTo={successLink} />
        </div>
      </div>
    )
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-slate-50">
      <div className="w-full max-w-[440px]">
        <div className="rounded-sm border border-slate-200 overflow-hidden bg-white">
          <div className="px-6 py-4 flex items-center justify-between" style={{ background: '#009CE1' }}>
            <div className="flex items-center gap-2.5">
              <img src="/logo2.png" alt="mendunia.id" className="h-5 w-auto brightness-0 invert opacity-90" />
          ...
          <div className="px-7 py-7">
            <div className="mb-6">
              <h2 className="text-[18px] font-bold text-slate-800">{title || 'Daftar'}</h2>
              <div className="mt-2 h-[3px] w-10 rounded-full" style={{ background: '#009CE1' }} />
              <p className="text-[12px] text-slate-500 mt-2">Lengkapi data berikut untuk mendaftar</p>
            </div>
          ...
          <div className="px-7 py-4 bg-slate-50 border-t border-slate-100 text-center">
            <p className="text-[12px] text-slate-400">
              Sudah memiliki akun?{' '}
              <Link to="/login" className="font-semibold hover:underline" style={{ color: '#009CE1' }}>
                Masuk
              </Link>
            </p>
          </div>
        ...
        <p className="text-[11px] text-slate-400 text-center mt-4">&copy; 2026 mendunia.id</p>
      </div>
    </div>
  )
}
      ...
      <div className="min-h-screen flex flex-col items-center justify-center bg-white selection:bg-gray-50 p-4">
        <div className="w-full max-w-[380px]">
          <div className="mb-10 text-center">
            ...
            <h1 className="text-[26px] font-light text-gray-900 tracking-tight mb-1.5">{title || 'Daftar'}</h1>
          ...
          <div className="mt-8 text-center border-t border-gray-100 pt-6">
            <p className="text-xs text-gray-400 font-light">
              Sudah memiliki akun?{' '}
              <Link to="/login" className="text-gray-900 font-medium hover:underline">Masuk</Link>
            </p>
          </div>
        </div>
      </div>
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-background selection:bg-primary/20 p-4">
      <div className="w-full max-w-[420px] animate-in fade-in slide-in-from-bottom-4 duration-700">
        <div className="flex flex-col items-center mb-6">
          <Link to="/login"><img src="/logo2.png" alt="mendunia.id" className="h-8 w-auto mb-1 opacity-90" /></Link>
          <p className="text-sm text-muted-foreground mt-2">{title || 'Daftar'}</p>
        </div>
